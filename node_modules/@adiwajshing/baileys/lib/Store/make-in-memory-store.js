    ...
    return {
        ...
        writeToMultiFiles: (folder) => {
            ...
            if (folder.includes('.json')) {
                ...
                writeFileSync(folder + 'chats.json', JSON.stringify(chats));
                writeFileSync(folder + 'contacts.json', JSON.stringify(contacts));
                writeFileSync(folder + 'messages.json', JSON.stringify(messages));
            }
        },
        readFromMultiFiles: (folder) => {
            // require fs here so that in case "fs" is not available -- the app does not crash
            const { readFileSync, existsSync } = require('fs');
            if (folder.includes('.json')) {
                readFileSync(folder);
                ...
                if (existsSync(folder)) {
                    ...
                    const chats = JSON.parse(readFileSync(folder + 'chats.json', { encoding: 'utf-8' }));
                    const contacts = JSON.parse(readFileSync(folder + 'contacts.json', { encoding: 'utf-8' }));
                    const messages = JSON.parse(readFileSync(folder + 'messages.json', { encoding: 'utf-8' }));
                    fromJSON({
                        chats: chats,
                        contacts: contacts,
                        messages: messages,
